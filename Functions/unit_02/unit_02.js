...
function t1(min, max) {
    ...
    document.querySelector('.out-1').innerHTML = rand;
}
...
function t9(text = '', block) {
    if (block) block.innerHTML = text.trim().toLowerCase();
}
...
function t12(...args) {
    //let sum = Array.from(arguments).reduce((accum, item) => accum += item); //перевод в массив
