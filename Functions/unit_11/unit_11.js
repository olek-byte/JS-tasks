...
function t1(elem) {
    ...
    function r1(element) {
        let innerEl = element.children;
        console.log(elem);
        Array.from(innerEl).forEach(item => {
            out += item.nodeName + ' ';
            r1(item);
        });
    }
...
function t2(elem) {
    ...
    function r2(element) {
        ...
        if (innerEl.length > 0) {
            Array.from(innerEl).forEach((item) => r2(item))
        }
...
function t4(elem) {
    ...
    function r4(element) {
        ...
        Array.from(collection).forEach(item => r4(item));
    }
}
...
function t5(elem) {
    ...
    function r5(element) {
        ...
        let collection = element.children;
        Array.from(collection).forEach(item => r5(item));
    }
}
