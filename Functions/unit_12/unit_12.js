...
/////поправил, правильно ли? как ее замыкать?
...
function generatePassword2(arr, n) {
    ...
    return function () {
        for (let i = 0; i < n; i++) {
            out += ar[randomInteger(0, ar.length - 1)];
        }
        return out;
    }
}
...
function generateElement(elem, text) {
    return function () {
        let newTag = document.createElement(elem);
