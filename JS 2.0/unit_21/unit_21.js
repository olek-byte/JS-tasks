...
function t4() {
    div4.addEventListener('touchstart', t4_1);
}
...
function t8() {
    ...
    let a8Rand = Math.floor(Math.random() * a8.length);
    this.style.backgroundColor = a8[a8Rand];
}
...
function t11(event) {
    let myObj = event.touches;
    let radiusX = myObj[0].radiusX;
    let radiusY = myObj[0].radiusY;
    document.querySelector('.out-11').innerHTML = `radiusX - ${radiusX} radiusY - ${radiusY}`;
}
...
// document.querySelector('.out-12').prepend(reset);
