...
const f13 = () => {
    ...
    document.querySelector('.out-13').innerHTML = max.length;
}
...
const f14 = () => {
    let res = a14.reduce((accum, item) => {
        if (item.length > accum.length) accum = item;
        return accum;
    }, a14[0]);
...
const f15 = () => {
    ...
    let average = res / a15.length;
    console.log(average);
    document.querySelector('.out-15').innerHTML = average;
}
...
const f20 = () => {
    out20.innerHTML = a20.reduce((accum, item) => {
        accum = item + ' ' + accum;
        return accum;
    }, ' ');
    console.log(out20);
