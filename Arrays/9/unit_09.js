...
let a2 = [13, 15, 22, 23, 263, 27, 35, 72];
let a2_from = 23;
let a2_to = 27;
...
const f4 = () => {
    let res = str4.split('');
    let find = res.find(elem => {
        if (elem > a4_from && elem < a4_to) {
            return true;
        }
    });
    console.log(find);
    document.querySelector('.out-4').innerHTML = find;
}
...
const f6 = () => {
    ...
    let temp = [...a6].reverse();
    let index = temp.findIndex(item => (item > a6_from && item < a6_to));
    out.innerHTML = (index === -1 ? -1 : a6.length - 1 - index);
}
...
const f8 = () => {
    let b = str8.lastIndexOf(a8_1.toUpperCase());
